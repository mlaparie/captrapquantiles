...
- If a row is missing (no row for the date), then the trap did not send data that day, which does not inform on whether some captures occurred. It would be a true "NA".
- If a row with a date exists but there is no count data in the second column, then the trap successfully communicated that day (battery or GPS coordinates for example), but no catch was detected.
- If a row with a date exists and count data is "0", then some activity was detected in the sensor but it was not attributed to the target species by the algorhythm.